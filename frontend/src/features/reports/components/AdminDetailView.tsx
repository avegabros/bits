                    ...
                    <table className="w-full text-left text-sm bg-white min-w-[820px]">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {sortedData.length === 0 ? (
                                ...
                                (() => {
                                    const isSortedByDate = sortKeyStr === 'loopDateStr' || sortKeyStr === null;
                                    const isAscending = sortOrder === 'asc';

                                    return sortedData.map((row) => {
                                        const { loopDate, loopDateStr, record, statusType, missingStatus, isFuture, checkInVal: checkIn, checkOutVal: checkOut, workedHrsVal: hoursWorked, lateMinsVal: lateMins, otMinsVal: otMins, utMinsVal: utMins, holidayName, shift, shiftChanged } = row;

                                        const showShiftChangeBanner = isSortedByDate && shiftChanged && shift;

                                        const renderShiftChangeBanner = () => {
                                            if (!shift) return null;
                                            return (
                                                <tr key={`shift-change-${loopDateStr}`} className="bg-slate-50/80 border-y border-slate-200/40">
                                                    <td colSpan={9} className="px-5 py-3 text-center">
                                                        <div className="flex items-center justify-center gap-2">
                                                            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase bg-indigo-50 text-indigo-700 border border-indigo-100 tracking-wider">
                                                                <Clock className="w-3.5 h-3.5 text-indigo-500" />
                                                                Shift Changed: {shift.name} ({formatShiftTime(shift.startTime)} – {formatShiftTime(shift.endTime)})
                                                            </span>
                                                        </div>
                                                    </td>
                                                </tr>
                                        ...
                                        return (
                                            <React.Fragment key={loopDateStr}>
                                                {showShiftChangeBanner && isAscending && renderShiftChangeBanner()}
                                                {renderDailyRow()}
                                                {showShiftChangeBanner && !isAscending && renderShiftChangeBanner()}
                                            </React.Fragment>
                                        );
