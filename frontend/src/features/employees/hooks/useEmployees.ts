  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter((emp) => {
      ...
      const matchesSearch =
        ...
        (emp.EmployeeShift?.some((es: any) => es.shift?.name?.toLowerCase().includes(searchStr) || es.shift?.shiftCode?.toLowerCase().includes(searchStr)) || false) ||
      ...
      const matchesShift = selectedShift === 'all' || emp.Shift?.name === selectedShift || (emp.EmployeeShift?.some((es: any) => es.shift?.name === selectedShift) || false);
