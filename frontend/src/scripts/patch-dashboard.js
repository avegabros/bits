...
const file = path.join('c:/bits/frontend/src/features/employee-portal/components/ShiftDetailsDashboard.tsx');
...
// We need to extract the render logic for a single shift into a function or map over it
const oldRender = `      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden relative">
...
const newRender = `      <div className="space-y-6">
        {(shifts && shifts.length > 0 ? shifts : [shift]).map((s, idx) => {
          const workDaysArray = (() => {
            try { return JSON.parse(s.workDays) }
            catch { return [] }
          })()

          return (
            <div key={s.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden relative">
              <div className={\`absolute top-0 left-0 w-full h-2 \${s.isNightShift ? 'bg-indigo-600' : 'bg-amber-400'}\`} />
              
              <div className="p-6 md:p-8">
                <div className="flex items-start justify-between mb-8 pb-8 border-b border-slate-100">
                  <div>
                    <div className="flex items-center gap-3 mb-2">
                      <h2 className="text-3xl font-black text-slate-900">{s.name}</h2>
                      <div className="flex items-center gap-2">
                        {idx === 0 && (shifts && shifts.length > 1) && (
                          <span className="px-2 py-1 text-[10px] font-black uppercase tracking-widest rounded-full bg-red-100 text-red-700">Primary</span>
                        )}
                        <span className={\`px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full \${
                          s.isNightShift ? 'bg-indigo-100 text-indigo-700' : 'bg-amber-100 text-amber-700'
                        } flex items-center gap-1.5\`}>
                          {s.isNightShift ? <Moon className="w-3 h-3" /> : <Sun className="w-3 h-3" />}
                          {s.isNightShift ? 'Night Shift' : 'Day Shift'}
                        </span>
                      </div>
                    </div>
                    <p className="text-slate-500 font-mono text-sm">Code: {s.shiftCode}</p>
                    {s.description && (
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-6 relative">
                      ...
                      <div className="relative flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Start Time</p>
                          <p className="text-2xl font-black text-slate-800">{formatTime(s.startTime)}</p>
                        </div>
                      </div>
                      ...
                      <div className="relative flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest flex gap-2 items-center">
                              Break Duration <span className="bg-slate-200 text-slate-600 px-2 py-0.5 rounded text-[10px]">{s.breakMinutes} mins</span>
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="relative flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">End Time</p>
                          <p className="text-2xl font-black text-slate-800">{formatTime(s.endTime)}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-8">
                    ...
                    <div>
                        ...
                        <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 flex items-center gap-4">
                          <Clock className="w-8 h-8 text-amber-500 opacity-80" />
                          <div>
                            <p className="font-bold text-slate-800 text-lg">{s.graceMinutes} minutes</p>
                            <p className="text-xs text-slate-500">Late Grace Period</p>
                          </div>
                        </div>
                    </div>
...
// Also remove the old \`const workDaysArray = ...\` from the top of the component
const oldWorkDaysArray = `  const workDaysArray = (() => {
    try { return JSON.parse(shift.workDays) }
    catch { return [] }
  })()\n\n`;
content = content.replace(oldWorkDaysArray, "");

fs.writeFileSync(file, content);
console.log("Successfully patched ShiftDetailsDashboard.tsx");
